                                                    ...
                                                    <td>
                                                        @if ($row->status == 0)
                                                            <button class="btn btn-sm btn-success" id="statusChangeEPIN"
                                                                data-id="{{ $row->balanceid }}" data-status="1">
                                                                <i class="fas fa-check"></i>
                                                            </button>
                                                            <button class="btn btn-sm btn-warning" id="statusChangeEPIN"
                                                                data-id="{{ $row->balanceid }}" data-status="2">
                                                                <i class="fas fa-times"></i>
                                                            </button>
                                                        @endif
                                                        <a href="{{ route('admin.request.balance.detail', $row->balanceid) }}"
                                                            class="btn btn-sm btn-primary">
                                                            <i class="far fa-eye"></i>
                                                        </a>
                                                        <button class="btn btn-sm btn-danger" id="deleteEPIN"
                                                            data-id="{{ $row->balanceid }}">
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            var url = '{{ url('/admin/request/balance/delete') }}' + '/' + id;
                            $.ajax({
                                url: url,
                                type: 'DELETE',
                                dataType: 'json',
            ...
            $("#table-1").on("click", "button#statusChangeEPIN", function() {
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            var token = $("meta[name='csrf-token']").attr("content");
                            var url = '{{ url('/admin/request/balance/change/status') }}';
                            $.ajax({
                                url: url,
                                type: 'POST',
                                dataType: 'json',
